...
仓库已包含实现和文档，单元测试全部通过。但在你自己的 AList 完成灰度验证前，请不要把远端修改流程当作可发布版本。
...
在同一个个人媒体库反复处理时，用户可以每个会话选择一次这个模式，免除重复的逐项确认。只有身份唯一明确、计划没有目标冲突、实时目录结构符合计划、且置信度达到计划阈值时，Skill 才能免去逐项确认、自动批准并执行**视频文件名**改动。

文件夹改名和移入 `Movies/`、`TV Shows/` 的目录移动在这个模式下都不会自动执行。文件夹改名会改写其下所有子路径，让此前视频日志里记录的路径失效，因此这两类改动都要等一次覆盖整批的明确确认：用户审阅本批全部待改名和待移动的文件夹，然后给出一次合并的放行。
...
计划会解析中文片名、去掉字幕和压制组噪声，并保留来源可佐证的发布组。剧集名 `Homecoming` 取自 TMDB 的 `en-US` 分集列表，是必填字段——源文件名没有也要补上。剧集年份与 ID 放在剧集目录和计划中；单集保留剧名与规范化后的 `SxxEyy` 键。
