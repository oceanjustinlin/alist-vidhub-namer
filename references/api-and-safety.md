...
To bring an already-organized season's loose `SxxEyy` files (sitting directly in a series folder because they were the show's only season at the time, or because an earlier batch skipped the wrapper) into the canonical `Season NN/` layout described in [naming.md](naming.md), use `season-wrap-plan` containing 1-20 exact source files, a `--season` number, and an absolute `--path` pointing at the series container. Every listed file must be a video or subtitle file carrying an `SxxEyy` marker for that same season; artwork, `.nfo` and `.url` files carry those markers too, so the marker alone does not qualify a file. That gate lives in the plan validator rather than only where the plan is built, so `season-wrap-apply` re-applies it to the plan file it actually loads. Separately, both `season-wrap-plan` and `season-wrap-apply` refuse to run while any other same-season video or subtitle is still loose under `--path` and absent from the list — apply re-runs that scan against its own fresh listing, so a sidecar that arrives after the plan was written, or one deleted from the plan by hand, still stops the move rather than being stranded. It only ever creates a brand-new `Season NN` subfolder directly under `--path`; it never targets one that already exists, so re-run it once per season that needs wrapping. `season-wrap-apply` re-lists the source directory, confirms the season folder is still absent, creates it, then moves each file into it one at a time with the same journal-per-mutation and automatic-restore-on-failure behavior as `organize-apply`. `season-wrap-rollback` mirrors `organize-rollback`: it moves files back to the series folder one at a time and never deletes the now-empty `Season NN` folder it created.

When video names, media folders, and organization have all changed, roll back in reverse dependency order: organization move, media-folder rename, video rename. A season wrap sits alongside organization in that ordering — undo it (`season-wrap-rollback`) before any folder-name rollback whose journal predates it.
